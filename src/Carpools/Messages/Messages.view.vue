    ...
    <ion-content color="primary" no-bounce>
      <div class="mc-white-container mc-white-container-no-padding">
        ...
        <div class="d-flex justify-between top-buttons-padding">
          <ion-button class='mc-small-button' color="primary" :fill="direct ? 'outline' : 'solid'" @click="direct = false">
            <ion-icon name="car"></ion-icon> <span class="ion-margin-start">{{ $t('Messages.carpool') }}</span>
          </ion-button>
          <ion-button class='mc-small-button' color="primary" :fill="direct ? 'solid' : 'outline'" @click="direct = true">
            <ion-icon name="chatbubbles"></ion-icon> <span class="ion-margin-start">{{ $t('Messages.direct') }}</span>
          </ion-button>
        </div>
        ...
        <div class="ion-text-center ion-margin-top" v-if="(direct ?  $store.state.messageStore.statusMessagesDirect == 'success' : $store.state.messageStore.statusMessagesCarpool == 'success') && messages.length == 0">
          {{ $t('Messages.none')}}
        </div>
        ...
        <ion-list v-if="direct ?  $store.state.messageStore.statusMessagesDirect == 'success' : $store.state.messageStore.statusMessagesCarpool == 'success'">
          <ion-item-sliding v-for="message in messages" :key="message.id">
            <ion-item class="message-list" @click="goToMessage(message)">
              ...
              <ion-label>
                <p class="message-name"><b>{{message.givenName}} {{message.shortFamilyName}}</b></p>
  ...
  export default {
    ...
    methods: {
      ...
      deleteThread(messageId) {
        this.$store.dispatch('deleteThread', messageId).then(res => {
